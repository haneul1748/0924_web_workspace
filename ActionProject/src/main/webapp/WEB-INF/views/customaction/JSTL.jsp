...
<body>
	...
	<p>
		Jakarta Server Page Standard Tag Library
	...
	2) Standard-1.2.5.jar파일 4개다 다운 <br>
	...
	<pre>
		솔직히 이야기하겠습니다. 사실 그냥 아까배운 Scope들에 새로운 속성 추가하는 방법입니다.
	...
	<!-- request쪽 영역만 삭제되었기때문에 session쪽 스코프 영역은 삭제되지않음. -->
	
	<hr>
	
	<pre>
		* 속성 출력 (&lt;c:out value="출력할값" default="기본값" excapeXml="t/f" />)
		- 데이터를 출력하려고 할 때 사용하는 태그
		- default : 기본값, value속성에 출력하고자 하는 값이 없을 경우 대체해서 출력할 
		            내용물을 쓸 수 있음(생략가능)
		- escapeXml : HTML태그를 인식시킬수있음
	</pre>
	...
	out태그를 써볼까? : <c:out value="${ requestScope.result }" default="값이 없음" />
	...
	<!-- choose안에 when/otherwise말고 들어가면 안됨 -->
		...
		<tbody>
			<c:choose>
				<c:when test="${ empty list }">
					<tr>
						<th colspan="3">조회결과가 존재하지않습니다.</th>
					</tr>
				</c:when>
				<c:otherwise>
