...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// case 3. 첨부파일이 O, 기존 첨부파일 X => BOARD UPDATE + AT INSERT
			...
			if(multiRequest.getOriginalFileName("reUpfile") != null) {
				...
				// 새로운 첨부파일이 존재하면 객체 생성후 원본명, 바꾼명, 경로 담기
				at = new Attachment();
			...
			if(result > 0 ) {
				session.setAttribute("arlerMsg", "게시글 수정 성공~");
				...
				request.getRequestDispatcher("/WEB-INF/common/result_page.jsp").forward(request, response);
